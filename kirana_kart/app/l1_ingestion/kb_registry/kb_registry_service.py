...
from app.operational_metrics import observed
...
class KBRegistryService:
    ...
    @staticmethod
    def _activate_version(conn, version_label: str) -> None:
        """
        Make `version_label` the live policy.

        Two tables have to agree for the Cardinal runtime to accept a policy:

          * kb_runtime_config.active_version — which version to load
          * policy_versions.is_active        — confirmation it may be used

        phase4_enricher checks BOTH and raises EnrichmentError when the second
        is false. Nothing used to set it: compiler_service inserts the row with
        is_active = FALSE and no code path ever flipped it, so every published
        policy failed enrichment on every ticket. Both writes now happen here,
        in the caller's transaction, and cannot drift apart.
        """
        # Exactly one version may be active.
        conn.execute(text("""
            UPDATE kirana_kart.policy_versions
            SET is_active = FALSE
            WHERE is_active = TRUE AND policy_version <> :version_label
        """), {"version_label": version_label})
        ...
        activated = conn.execute(text("""
            ...
            WHERE policy_version = :version_label
        """), {"version_label": version_label}).rowcount

        if not activated:
            raise Exception(
                f"Policy version '{version_label}' has no policy_versions row — "
                "it must be compiled before it can be activated."
            )

        existing_runtime = conn.execute(text("""
            SELECT id FROM kirana_kart.kb_runtime_config
            ORDER BY id DESC LIMIT 1
        """)).scalar()

        if existing_runtime:
            # Scoped to the current row: the read path elsewhere uses
            # ORDER BY id DESC LIMIT 1, so an unqualified UPDATE would rewrite
            # history rows too.
            conn.execute(text("""
                ...
                SET active_version = :version_label,
                    activated_at = NOW()
                WHERE id = :id
            """), {"version_label": version_label, "id": existing_runtime})
        else:
            conn.execute(text("""
                INSERT INTO kirana_kart.kb_runtime_config (active_version, activated_at)
                VALUES (:version_label, NOW())
            """), {"version_label": version_label})
    ...
    def rollback(
        ...
        with self.engine.begin() as conn:
            ...
            # Rolling back has to flip policy_versions.is_active too, or the
            # runtime rejects the version we just rolled back to.
            self._activate_version(conn, target_version)
