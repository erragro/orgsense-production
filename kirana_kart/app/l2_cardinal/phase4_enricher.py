...
from app.utils.customer_pii import email_index, decode
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    f"WHERE (email_blind_index = %s OR (email_blind_index IS NULL AND lower(trim(email)) = %s)) LIMIT 1",
                    (email_index(canonical.cx_email), canonical.cx_email.strip().lower()),
                )
                row = cur.fetchone()
...
def _fetch_customer_profile(
    ...
    try:
            ...
            if customer_id:
                ...
                cur.execute(
                    ...
                    SELECT
                        ...
                        customer_churn_probability, signup_date
                    FROM {SCHEMA}.customers
                    WHERE (email_blind_index = %s OR (email_blind_index IS NULL AND lower(trim(email)) = %s))
                    LIMIT 1
                    """,
                    (email_index(cx_email), cx_email.strip().lower()),
                )
            row = cur.fetchone()
        ...
        raise EnrichmentError("Customer profile is unavailable") from exc
    finally:
        conn.close()
...
def _fetch_risk_profile(customer_id: Optional[str]) -> RiskProfile:
    ...
    try:
        ...
        raise EnrichmentError("Customer risk profile is unavailable") from exc
    finally:
        conn.close()
