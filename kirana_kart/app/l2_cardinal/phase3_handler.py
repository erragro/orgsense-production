...
import psycopg2
from app.admin.db import get_connection as pooled_connection
...
def _verify_source(
    ...
    # Sandbox: skip source-specific verification.
    #
    # `is_sandbox` derives from metadata.test_mode / the org name, both of
    # which come from the request body — POST /cardinal/ingest is a public
    # webhook endpoint, so an anonymous caller could previously set
    # {"org": "sandbox"} and inject tickets with no credential at all.
    # Sandbox now only waives verification outside production; a production
    # deployment still requires a valid API token for sandbox traffic.
    if is_sandbox:
        if not settings.is_production:
            return True, "skipped", "Source verification skipped — sandbox mode."
        verified, _method, _warn = _verify_api_token(auth_token)
        if verified:
            return (
                True,
                "sandbox_token",
                "Sandbox request accepted on API token — verification skipped.",
            )
        return False, "sandbox_token", None

    if source == "freshdesk":
        return _verify_freshdesk_signature(raw_body, signature_header)

    elif source in ("api", "webhook"):
        return _verify_api_token(auth_token)

    elif source == "gmail":
        # Gmail OAuth2 verification is not implemented. Returning verified=True
        # unconditionally made {"source": "gmail"} an authentication bypass on
        # a public endpoint. Fall back to the API token: a caller that holds a
        # valid token is allowed through with the gap recorded, and everyone
        # else is denied.
        verified, _method, _warn = _verify_api_token(auth_token)
        if verified:
            return (
                True,
                "gmail_token_fallback",
                "Gmail OAuth2 verification not implemented — accepted on API token.",
            )
        return False, "gmail_unimplemented", None
...
def _verify_freshdesk_signature(raw_body: Optional[bytes], signature_header: Optional[str] = None) -> tuple[bool, str, Optional[str]]:
    """Fail closed unless the exact raw payload has a valid HMAC-SHA256."""
    if not FRESHDESK_WEBHOOK_SECRET or not raw_body or not signature_header:
        return False, "freshdesk_signature", None
    expected = hmac.new(FRESHDESK_WEBHOOK_SECRET.encode(), raw_body, hashlib.sha256).hexdigest()
    supplied = signature_header.removeprefix("sha256=").strip().lower()
    # Compare only a fixed-size hex digest; malformed headers cannot throw.
    if len(supplied) != 64 or any(c not in '0123456789abcdef' for c in supplied):
        return False, "freshdesk_signature", None
    return hmac.compare_digest(expected, supplied), "freshdesk_signature", None
